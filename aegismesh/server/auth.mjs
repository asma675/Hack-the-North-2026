...
export async function ensureDemoUser(){ return mutate(state=>{ let u=state.users.find(x=>x.email==='judge@aegismesh.ai'); if(!u){ const {salt,hash}=hashPassword('demo1234'); u={id:newId('usr'),email:'judge@aegismesh.ai',role:'admin',verified:true,salt,passwordHash:hash,created_at:new Date().toISOString()}; state.users.push(u); state.entities.User.push({id:u.id,email:u.email,role:u.role,created_date:u.created_at}); } return u; }); }
