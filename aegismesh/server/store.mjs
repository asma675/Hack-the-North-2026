...
export async function loadState(){
  if(cfEnv?.AEGIS_KV){try{const raw=await cfEnv.AEGIS_KV.get('state:global');if(raw)return JSON.parse(raw);}catch(e){console.warn('KV read failed, using memory:',e.message);}}
...
export async function persistState(state){
  ...
  if(cfEnv?.AEGIS_KV){try{await cfEnv.AEGIS_KV.put('state:global',JSON.stringify(state));return;}catch(e){console.warn('KV write failed, writing local:',e.message);}}
